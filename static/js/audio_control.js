...
const audioSlug = window.location.pathname.slice(window.location.pathname.lastIndexOf('track/')).replace('track/', '').replace('/', '')
...
async function firstPlay() {
    const url = `http://${window.location.host}/player/api/v1/track/metadata/${audioSlug}/`
    const responseMetadata = await fetch(url)
    const metadata = await responseMetadata.json()
    const audioUrl = `http://${window.location.host}/player/api/v1/track/${audioSlug}/`
...
async function changeSongFetch(data) {
    if (!isRepeat) {
        const url = `http://${window.location.host}/player/api/v1/metadata/next/${audioSlug}/`
